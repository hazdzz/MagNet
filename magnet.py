...
def get_parameters():
    ...
    parser.add_argument('--model', type=str, default='magnet', help='graph neural network model')
    parser.add_argument('--gso_type', type=str, default='sym_renorm_mag_adj', \
                        choices=['sym_renorm_mag_adj', 'rw_renorm_mag_adj'], \
    ...
    if args.gso_type != 'sym_renorm_mag_adj' and args.gso_type != 'rw_renorm_mag_adj':
    ...
    model_save_path = model_name + '_' + gso_type + '_' + str(q) + '_q_' + str(K) + '_layer' + '.pth'
    model_save_path = os.path.join(model_save_dir, model_save_path)

    return device, dataset, model_name, gso_type, lr, weight_decay, droprate, n_hid, enable_bias, q, K, epochs, opt, patience, model_save_path
    
def process_data(device, dataset, gso_type, q):
    ...
    if q != 0 and q != 0.5:
        ...
        gso_real = gso
        if sp.issparse(gso):
            gso_imag = sp.identity(gso.shape[0], format='csc')
        else:
            gso_imag = np.identity(gso.shape[0])
    ...
    gso_imag = utility.cnv_sparse_mat_to_coo_tensor(sp_mat=gso_imag, dtype=np.float32, device=device)
    label = torch.LongTensor(label).to(device)

    return gso_real, gso_imag, feature, label, idx_train, idx_val, idx_test, n_feat, n_class

def prepare_model(q, n_feat, n_hid, n_class, enable_bias, K, droprate, patience, model_save_path, opt, lr, weight_decay):
    if q == 0 or q == 0.5:
        model = models.RMagNet(n_feat, n_hid, n_class, enable_bias, K, droprate).to(device)
    else:
        model = models.CMagNet(n_feat, n_hid, n_class, enable_bias, K, droprate).to(device)
...
def train(epochs, model, optimizer, scheduler, early_stopping, feature, gso_real, gso_imag, label, loss, idx_train, idx_val):
    train_time_list = []
    for epoch in range(epochs):
        train_epoch_begin_time = time.perf_counter()
        model.train()
        optimizer.zero_grad()
        output = model(feature, gso_real, gso_imag)
...
def test(model, model_save_path, feature, gso_real, gso_imag, label, loss, idx_test, model_name, dataset, mean_train_epoch_time_duration):
...
if __name__ == "__main__":
    device, dataset, model_name, gso_type, lr, weight_decay, droprate, n_hid, enable_bias, q, K, epochs, opt, patience, model_save_path = get_parameters()
    gso_real, gso_imag, feature, label, idx_train, idx_val, idx_test, n_feat, n_class = process_data(device, dataset, gso_type, q)
    model, loss, early_stopping, optimizer, scheduler = prepare_model(q, n_feat, n_hid, n_class, enable_bias, K, droprate, patience, model_save_path, opt, lr, weight_decay)
    mean_train_epoch_time_duration = train(epochs, model, optimizer, scheduler, early_stopping, feature, gso_real, gso_imag, label, loss, idx_train, idx_val)
    test(model, model_save_path, feature, gso_real, gso_imag, label, loss, idx_test, model_name, dataset, mean_train_epoch_time_duration)